export let nowShowing = [
  {
    id: "s1",
    name: "Bombshell",
    language: "English",
    genre: "Commercial",
    format: "2D",
    src: require("../assets/img/bombshell.jpg"),
    bgsrc: require("../assets/img/grudge_bg.jpg"),
    synopsis:
      "Bombshell is a 2019 American biographical drama film directed by Jay Roach and written by Charles Randolph. The film stars Charlize Theron, Nicole Kidman, and Margot Robbie, and is based upon the accounts of the women at Fox News who set out to expose CEO Roger Ailes for sexual harassment. Actors John Lithgow, Kate McKinnon, Connie Britton, Malcolm McDowell, and Allison Janney appear in supporting roles.",
  },
  {
    id: "s2",
    name: "The Grudge",
    language: "Hindi",
    genre: "Horror",
    format: "2D",
    src: require("../assets/img/thegrudge.jpg"),
    bgsrc: require("../assets/img/grudge_bg.jpg"),
    synopsis:
      "A detective investigates a murder scene that has a connection to a case that her new partner handled in the past. The killings occurred in a haunted house that passes on a ghostly curse to those who dare enter it. Soon, the curse spreads to a terminally ill woman and her husband, and another unsuspecting couple who were in the wrong place at the wrong time.",
  },
  {
    id: "s3",
    name: "Mafia",
    language: "Tamil",
    genre: "Action",
    format: "2D",
    src: require("../assets/img/mafia.jpg"),
    bgsrc: require("../assets/img/mafia.jpg"),
    synopsis:
      "Set in Chennai, Mafia Chapter 1 is a high octane gangster drama that portrays a cat and mouse game between two powerful characters. This captivating and gritty narrative will keep you on the edge of your seat.",
  },
  {
    id: "s4",
    name: "Darbar",
    language: "Tamil",
    genre: "Commercial",
    format: "2D",
    src: require("../assets/img/darbar.png"),
    bgsrc: require("../assets/img/darbar.png"),
    synopsis:
      "A police officer on a chase to hunt down a dreaded gangster for fulfilling his own secret agenda.",
  },
  {
    id: "s5",
    name: "Oh My Kadavuley",
    language: "Tamil",
    genre: "Comedy",
    format: "2D",
    src: require("../assets/img/OMK.jpg"),
    bgsrc: require("../assets/img/OMK.jpg"),
    synopsis:
      "Ashok and Ritika are friends who decide to marry each other, but married life is not what they expected. As they get stuck trying to spice up their romantic lives, Meera - Ashok`s former crush - turns up and complicates things further.",
  },
  {
    id: "s6",
    name: "Sonic The Hedgehog",
    language: "English",
    genre: "Action",
    format: "2D",
    src: require("../assets/img/sonic-the-hedgehog.jpg"),
    bgsrc: require("../assets/img/sonic-the-hedgehog.jpg"),
    synopsis:
      "Based on the popular video game, Sonic The Hedgehog is the first entry in the film series, with its plot revolving around Sonic and his friends, who team up in order to beat the evil Mr. Eggman.",
  },
  {
    id: "s7",
    name: "The Invisible Man",
    language: "English",
    genre: "Horror",
    format: "2D",
    src: require("../assets/img/the-invisible-man.png"),
    bgsrc: require("../assets/img/the-invisible-man.png"),
    synopsis:
      "The extremely abusive nature of her boyfriend has forced Cecilia to run away. She finds out later that Adrian, her ex-boyfriend has committed suicide. However, Adrian fakes his death, becomes invisible and makes life a living hell for Cecilia. He influences her in a way that makes her look insane to everyone else around.",
  },
  {
    id: "s8",
    name: "Onward",
    language: "English",
    genre: "Disney",
    format: "3D",
    src: require("../assets/img/onward.png"),
    bgsrc: require("../assets/img/onward.png"),
    synopsis:
      "Set in a suburban fantasy world that has forgotten magic, two elf brothers receive a prearranged gift - a magic staff - from their father, who passed away many years ago. They try to use it to bring him back but end up conjuring only the lower half of his body. The two go on a quest in search of magic so they can bring all of him back, but they`ve only got 24 hours till the spell`s effect fades away.",
  },
  {
    id: "s9",
    name: "Baaghi-3",
    language: "Hindi",
    genre: "Action",
    format: "3D",
    src: require("../assets/img/baaghi-3.jpg"),
    bgsrc: require("../assets/img/baaghi-3.jpg"),
    synopsis:
      "Ronnie and Vikram are brothers who share an unbreakable bond. Since childhood, Ronnie always come to the rescue whenever Vikram falls in any trouble. Their journey begins when a certain turn in events, leads Vikram to travel abroad to complete some work. On this trip, Vikram gets kidnapped by people who are not to be messed with but as Ronnie witnesses, his brother getting beaten and kidnapped, he knows that he will do whatever it takes to destroy anyone and anything that stands in the way of Vikram`s safety. Ronnie goes on a rampage of destruction to see his brother safe again, even if it means that he independently has to take on an entire country.",
  },
];

export let comingSoon = [
  {
    id: "c1",
    name: "Jumanji-the-next-level",
    language: "English",
    genre: "Horror",
    format: "2D",
    src: require("../assets/img/jumanji-the-next-level.jpg"),
  },
  {
    id: "c2",
    name: "Spies-in-disguise",
    language: "English",
    genre: "Disney",
    format: "3D",
    src: require("../assets/img/spies-in-disguise-.jpg"),
    synopsis:
      "When a nurse downloads an app that claims to predict the moment a person will die, it tells her she only has three days to live. With the clock ticking and a figure haunting her, she must find a way to save her life before time runs out.",
  },
  {
    id: "c3",
    name: "Dhruva Natchathiram",
    language: "Tamil",
    genre: "Action",
    format: "2D",
    src: require("../assets/img/dhruva-natchathiram.jpg"),
  },
  {
    id: "c4",
    name: "Master",
    language: "Tamil",
    genre: "Commercial",
    format: "2D",
    src: require("../assets/img/master.jpg"),
  },
  {
    id: "c5",
    name: "Soorarai Pottru",
    language: "Tamil",
    genre: "Action",
    format: "2D",
    src: require("../assets/img/soorarai-pottru.png"),
  },
  {
    id: "c6",
    name: "Fast and Furious",
    language: "English",
    genre: "Action",
    format: "2D",
    src: require("../assets/img/fast-and-furious.jpg"),
  },
  {
    id: "c7",
    name: "Peter-Rabbit-2",
    language: "English",
    genre: "Disney",
    format: "3D",
    src: require("../assets/img/peter-rabbit-2.jpg"),
  },
];

export let exclusive = [
  {
    id: "e1",
    name: "Countdown",
    language: "English",
    genre: "Horror",
    format: "2D",
    src: require("../assets/img/countdown.png"),
  },
  {
    id: "e2",
    name: "Bunker",
    language: "English",
    genre: "Action",
    format: "2D",
    src: require("../assets/img/bunker.png"),
  },
];
export let theaterList = [
  {
    id: "t1",
    name: "Inox prozone mall",
    showTimings: [
      { time: "10.15 AM", seats: "Filled", rate: 92 },
      { time: "1.20 PM", seats: "Filling Fast", rate: 102 },
      { time: "4.30 PM", seats: "Available", rate: 120 },
      { time: "7.30 PM", seats: "Filled", rate: 120 },
    ],
  },
  {
    id: "t2",
    name: "Fun Republic mall",
    showTimings: [
      { time: "10.15 AM", seats: "Filled", rate: 92 },
      { time: "1.20 PM", seats: "Filling Fast", rate: 102 },
      { time: "4.30 PM", seats: "Available", rate: 120 },
      { time: "7.30 PM", seats: "Filled", rate: 120 },
    ],
  },
  {
    id: "t3",
    name: "Brook Fields mall",
    showTimings: [
      { time: "10.15 AM", seats: "Filled", rate: 92 },
      { time: "1.20 PM", seats: "Filling Fast", rate: 102 },
      { time: "7.30 PM", seats: "Filled", rate: 120 },
    ],
  },
  {
    id: "t4",
    name: "KG Cinemas",
    showTimings: [
      { time: "10.15 AM", seats: "Filled", rate: 92 },
      { time: "1.20 PM", seats: "Filling Fast", rate: 102 },
      { time: "4.30 PM", seats: "Available", rate: 120 },
      { time: "10.30 PM", seats: "Filled", rate: 120 },
    ],
  },
];
